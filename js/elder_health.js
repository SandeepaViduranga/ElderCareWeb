...
function loadElder() {
    ...
    $.ajax({
        ...
        $('#elders_health').empty();
        $('#elders_health').append("<thead></thead><tr><th>Elder's name</th><th>NIC</th><th>Age</th><th>Sugar Level<br> (mg/dL)</th><th>Pressure Level<br> (SBP)</th><th>Body Temp.<br>( °C )</th><th>BMI</th><th>Record Date</th></tr></thead><tbody>");

        result.forEach(function (result) {
            $('#elders_health').append('<tr><td>' + result.first_name + '</td><td>' + result.nic + '</td><td>' + result.age + '</td><td>' + result.sugar_level + '</td><td>' + result.pressure_level + '</td><td>' + result.body_temp + '</td><td>' + result.BMI + '</td><td>' + result.input_date +'</td></tr>');
        })
