...
function Admin_req() {
    ...
    $.ajax({
        ...
        result.forEach(function (result) {
            $('#doctor_req').append('<tr><td>' + result.Name + '</td><td>' + result.mobile + '</td><td>' + result.address + '</td><td>' + result.nic + '</td><td>' + result.email + '</td><td>' + result.mbbsid + '</td>' + updateDoctorBtn(result.nic) + '</tr>');
...
function updateDoctorBtn(str) {
    var temp = "";
    console.log(str);
    if (str != "0") {
        temp = '<td><a href="/ElderCareWeb/admin_warning.html?docNIC='+str+'" ><button type="button" class="btn btn-sm btn-gradient-success">Approve</button></a></td>';
    }
    return temp;
}

function doctor_Update_Status(Doc_ID) {
    //var Elder_ID = getElder_ID();
    console.log(Doc_ID);
    $.ajax({
        url: "PHP/admin_requests.php",
        method: "post",
        data: "doctor_Update_Status=" + Doc_ID
    }).done(function () {
